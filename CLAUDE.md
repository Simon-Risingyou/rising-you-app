...
- **(2026-07-01) Tegoed ongedaan maken (iedereen) + beurten aftrekken (admin)**. Migratie `0017_ongedaan_maken_en_aftrekken.sql`:
  ...
  - In de "Tegoed toevoegen"-modal (`ledenbeheer.html`): een "Ongedaan maken"-knopje verschijnt na het toevoegen van een beurtenkaart/abonnement (verwijst naar precies de zonet aangemaakte rij via `laatsteBeurtenkaartId`/`laatsteAbonnementId`, gereset bij elke keer dat de modal opent), en een apart blok "Beurten aftrekken (admin)" onderaan dat enkel zichtbaar is in admin-modus.
