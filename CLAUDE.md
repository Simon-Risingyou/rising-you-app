...
- **(2026-07-01) Distributie als installeerbare .exe + remote auto-updates, klaar voor klantdemo.** Op vraag ("dit is een versie die ik aan mijn klant wil laten zien... liefst 1 .exe... moet vanop afstand kunnen updaten") ingericht:
  - Twee echte build-blokkerende bugs gevonden en gefixt vóórdat er zelfs een installer gemaakt kon worden: (1) Vite's standaard esbuild-target ondersteunt geen top-level `await` (gebruikt in elk scherm) — `vite.config.js` kreeg `build.target: 'esnext'` (mag, want de app draait uitsluitend in Tauri's eigen moderne WebView2, nooit in willekeurige browsers). (2) `vendor/` (jsPDF, qrcode.min.js) werd nooit naar `dist/` gekopieerd — deze scripts worden via gewone (niet-module) `<script src="...">`-tags geladen, dus vielen ze buiten Vite's module-graaf; verplaatst naar `public/vendor/` (Vite's standaardplek voor statische bestanden die ongewijzigd meemoeten). Beide gecontroleerd: `npm run build` én `npm run dev` werken nog steeds correct erna.
  - `src-tauri/tauri.conf.json`: bundel-target vereenvoudigd tot enkel `nsis` (één Windows-installer-.exe; MSI is geschrapt, dat vereist het aparte WiX-toolset — niet nodig). `windows.nsis.installMode: "currentUser"` gekozen zodat de installatie **geen Windows-adminrechten/UAC-scherm** vraagt (installeert enkel voor de huidige gebruiker) — belangrijk voor een klant die zelf niet weet hoe.
  - **Remote updates**: `tauri-plugin-updater` was door een eerdere sessie al correct in `Cargo.toml`/`lib.rs`/`main.rs` gescaffold, enkel de configuratie in `tauri.conf.json` stond nog op placeholder-waarden. Nu gekoppeld aan `https://github.com/Simon-Risingyou/rising-you-app/releases/latest/download/latest.json`. Om updates te kunnen ondertekenen is een minisign-sleutelpaar aangemaakt (`npx tauri signer generate`, **zonder wachtwoord**, expliciete keuze van Simon): publieke sleutel staat in `tauri.conf.json` (`plugins.updater.pubkey`), de **private sleutel staat bewust NIET in de git-repo** maar lokaal op `C:\Users\simon\.tauri-keys\rising-you-updater.key`. **Belangrijk: Simon moet dit bestand ergens veilig back-uppen (bv. 1Password, dat hij al gebruikt voor de SSH-agent) — zonder deze sleutel kunnen toekomstige updates niet meer ondertekend/uitgerold worden.**
  - Build getest: `npm run tauri:build` (Rust release-compilatie, ~2 min) slaagt, installer staat op `src-tauri/target/release/bundle/nsis/Rising You Ledenbeheer_0.1.0_x64-setup.exe` (±3,6 MB). Dit ene bestand kan naar de klant gestuurd worden — dubbelklikken installeert de volledige app zonder verdere stappen/instellingen die de klant zou zien.
  - **Bekende, verwachte waarschuwing**: omdat de installer niet ondertekend is met een (betaald) code-signing-certificaat, kan Windows SmartScreen bij de eerste keer uitvoeren een blauw "Windows heeft de pc beschermd"-scherm tonen. Dit is normaal voor een nieuwe, nog onbekende .exe — de klant klikt op "Meer info" → "Toch uitvoeren". Dit is een apart probleem van de update-ondertekening hierboven (die gaat over het vertrouwen van *updates*, niet over het eerste-installatie-scherm van Windows zelf).
  - **Toekomstige update uitrollen (workflow voor Simon)**: versienummer verhogen op drie plekken tegelijk (`src-tauri/tauri.conf.json` → `version`, `package.json` → `version`, `src-tauri/Cargo.toml` → `version`), opnieuw `npm run tauri:build` draaien, en dan een nieuwe **GitHub Release** aanmaken op `Simon-Risingyou/rising-you-app` met daarin de nieuwe `...-setup.exe` én het bijhorende `latest.json`-manifest (Tauri's bundler genereert bij het bouwen automatisch een `.sig`-handtekeningsbestand naast de installer — nodig voor `latest.json`). Dit laatste (Release aanmaken) moet Simon zelf doen via GitHub Desktop of de GitHub-website, aangezien hier geen geauthenticeerde GitHub-toegang beschikbaar is om dat te automatiseren.
