...
- **(2026-07-01) Check-in: demo-namenrij onder "Scan kaartje" verwijderd.** `index.html` had een `<div id="demorij">` die bij het opstarten gevuld werd met een knopje per naam uit een oude, lokale test-dataset (`leden`-object met nep-codes als `"RY-guy"`), puur om tijdens de bouw snel een scan te kunnen simuleren. Div, CSS (`.demo-rij`) en de vulcode zijn verwijderd; verder niets aangeraakt.
  - **Ontdekt maar niet aangeraakt (buiten scope)**: die oude `leden`/`gezinnen`-mock-dataset, `verwerkScanCode()`, `tariefVan()`, `aboTotVoor()`, `beurtenVoor()`, `ledenById`, en een globale scan-buffer-listener (onderschept snel getypte tekst die met `"RY-"` begint) zijn nog aanwezig maar **niet meer aangesloten op de echte database** — vergelijkbaar met de dode `lidmaatschapSectieHTML()` die eerder al in `ledenbeheer.html` gevonden is. Ze zijn in de praktijk onschadelijk omdat echte QR-tokens UUID's zijn (nooit `"RY-"`-prefix), maar vormen samen een fors stuk ongebruikte legacy-code in `index.html`. Kan later opgeruimd worden.
